class WallpaperGenerator {
   ...
   generateGridLayout(resolution = "desktop") {
      ...
      positions.forEach((pos) => {
         ...
         // Draw image
         this.ctx.drawImage(pos.img, drawX, drawY, drawWidth, drawHeight);
   ...
   generateMasonryLayout(resolution = "desktop") {
      ...
      if (images.length === 1 && resolution === "mobile") {
         // New condition for a single image in mobile layout
         const img = images[0];
         const imgRatio = img.height / img.width;
         const fullWidth = this.canvas.width - padding * 2; // Full width minus padding
         const fullHeight = fullWidth * imgRatio; // Maintain aspect ratio

         this.drawImageInCell(img, padding, padding, fullWidth, fullHeight);
         return; // Exit the function after drawing the single image
      }
         ...
         while (!validPosition && attempts < 50) {
            ...
            for (const placed of placedImages) {
               const distance = Math.sqrt(
                  Math.pow(x + width / 2 - (placed.x + placed.width / 2), 2) + Math.pow(y + height / 2 - (placed.y + placed.height / 2), 2),
               );
